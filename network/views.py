...
def send_activation_email(user,request):
    ...
    email_body=render_to_string('network/activate.html',{
        'user':user,
        'domain':current_site,
...
def register(request):
    ...
    if request.method == "POST":
        ...
        #ensures the email is official dcrust email id
        elif (not email.endswith('dcrustm.org')):
            return render(request, "network/register.html", {
                "message": _("Only Official Dcrust Email ID Can Be Registered.")
            })
...
def activate(request,uidb64, token):
    ...
    if user and generate_token.check_token(user,token):
        ...
        return redirect(reverse("network:login"))
    
    return render(request,'network/activate-failed.html',{
        'user':user
    })